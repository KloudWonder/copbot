...
@client.tree.command(name="joinroleset", description="Sets Join Roles for When a Member Joins Your Server")
async def join_role_set(interaction: discord.Interaction, role_mentions: str):
    try:
        guild = interaction.guild
        join_roles_file = os.path.join(data_folder, f'{guild.id}_join_roles.json')

        role_names = [role_name.strip() for role_name in role_mentions.split(',')]
        roles_to_assign = []

        for role_name in role_names:
            role = discord.utils.get(guild.roles, name=role_name)
            if role:
                roles_to_assign.append({"name": role.name, "id": role.id})

        if roles_to_assign:
            server_join_roles = {'roles_to_assign': roles_to_assign}
            with open(join_roles_file, 'w') as file:
                json.dump(server_join_roles, file)

            role_info = "\n".join([f"{index + 1}. {role_data['name']} | ID: {role_data['id']}" for index, role_data in enumerate(roles_to_assign)])

            embed = discord.Embed(
                title="Join Roles Set!",
                description=f"The server's join roles have been set to:\n{role_info}",
                color=discord.Color.blue()
            )
            ...
            embed = discord.Embed(
                title="**ERROR**",
                description="No valid roles were mentioned.",
...
@client.tree.command(name="joinrolelist", description="List the Set Join Roles")
async def join_role_list(interaction: discord.Interaction):
    try:
        guild = interaction.guild
        on_join = os.path.join(data_folder, f'{guild.id}_on_join.json')

        if os.path.exists(join):
            with open(join, 'r') as file:
                server_join_roles = json.load(file)
                if 'roles_to_assign' in server_join_roles:
                    roles_to_assign = server_join_roles['roles_to_assign']
                    ...
                    embed = discord.Embed(
                        title="Join Roles List",
                        description="No join roles have been set.",
                        color=discord.Color.green()
                    )
                    embed.set_footer(text='CopBot',icon_url="https://cdn.discordapp.com/attachments/1152316262676963489/1156869427279691776/copbot_site_logo.png?ex=65168a1d&is=6515389d&hm=f941d50dfa5b2edef77d274e64894ae8e51bd3aad787792906e257d686aff533&")
                    await interaction.response.send_message(embed=embed)
...
@client.tree.command(name="joinroleremove", description="Remove a Join Role by Number")
async def join_role_remove(interaction: discord.Interaction, role_number: int):
    guild = interaction.guild
    on_join = os.path.join(data_folder, f'{guild.id}on_join.json')

    if os.path.exists(on_join):
        with open(join, 'r') as file:
            try:
                server_join_roles = json.load(file)
                if 'roles_to_assign' in server_join_roles:
                    ...
                    if 1 <= role_number <= len(roles_to_assign):
                        removed_role = roles_to_assign.pop(role_number - 1)

                        server_join_roles['roles_to_assign'] = roles_to_assign
                        with open(join, 'w') as update_file:
                            json.dump(join, update_file)

                        embed = discord.Embed(
                            title="Join Role Removed :heavy_check_mark:",
                            description=f"Removed role: {removed_role}",
                            color=discord.Color.red()
                        )
                        embed.set_footer(text='CopBot',icon_url="https://cdn.discordapp.com/attachments/1152316262676963489/1156869427279691776/copbot_site_logo.png?ex=65168a1d&is=6515389d&hm=f941d50dfa5b2edef77d274e64894ae8e51bd3aad787792906e257d686aff533&")
                        await interaction.response.send_message(embed=embed)
                        ...
                        embed = discord.Embed(
                            title="**ERROR**",
                            description="Invalid role number. Please specify a valid number.",
                            color=discord.Color.red()
                        )
                        embed.set_footer(text='CopBot',icon_url="https://cdn.discordapp.com/attachments/1152316262676963489/1156869427279691776/copbot_site_logo.png?ex=65168a1d&is=6515389d&hm=f941d50dfa5b2edef77d274e64894ae8e51bd3aad787792906e257d686aff533&")
                        await interaction.response.send_message(embed=embed)
                ...
                embed = discord.Embed(
                    title="**ERROR**",
                    description="There was an error while loading the join roles.",
                    color=discord.Color.red()
                )
                embed.set_footer(text='CopBot',icon_url="https://cdn.discordapp.com/attachments/1152316262676963489/1156869427279691776/copbot_site_logo.png?ex=65168a1d&is=6515389d&hm=yy")
                await interaction.response.send_message(embed=embed)
    else:
        embed = discord.Embed(
            title="**ERROR**",
...
@client.tree.command(name="invite",description="Invites The Premium Version Of CopBot To Your Server")
async def invite(interaction:discord.Interaction):
    client_id = 1149285691994407043

    permissions = discord.Permissions.all()
    invite_link = discord.utils.oauth_url(client_id, permissions=permissions)

    embed = discord.Embed(
        title="⚠️ **WARNING**",
        description=f"If you __do not__ own a premium subscription and have the role in our discord this bot will automatically leave your server! Invite Link ---> {invite_link}",
